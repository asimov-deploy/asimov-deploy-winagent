...
namespace AsimovDeploy.WinAgent.Framework.Tasks {
	...
	public class ChangeLoadBalancerStates : AsimovTask {
		...
		protected override string InfoString() {
			return
				string.Format(
					"Allow cookie persistent HTTP sessions: [{0}], Mark existing sessions for removal from session table: [{1}]",
					Config.LoadBalancer.AllowCookiePersistedSessions ? "y" : "n",
					Config.LoadBalancer.RemoveExistingConnectionsFromSessionTable ? "y" : "n");
		}
