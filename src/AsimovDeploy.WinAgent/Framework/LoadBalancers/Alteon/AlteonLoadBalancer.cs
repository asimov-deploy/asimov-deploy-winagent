			...
			while (!(s = _connection.ReadToEnd()).Contains(">>")) {
				if (s.Contains("Allow cookie persistent HTTP sessions ?")) {
					...
					throw new LoadBalancerCommunicationException(
						string.Format("Error while trying to disconnect server {0}, unrecognized response \"{1}\".", id, s));
				}
			}
		}
		...
		private class Connection : IDisposable {
			...
			private void EnsureConnected() {
				if (!_socket.Connected) {
					throw new LoadBalancerCommunicationException(
						"Error while reading from load balancer, expected to read some data from the socket, but the socket was in a faulted state.");
				}
			}
